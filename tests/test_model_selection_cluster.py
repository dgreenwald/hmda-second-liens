...
from hmda_seconds.logistic_features import core_specifications
...
def complete_coarse_summary() -> pd.DataFrame:
    ...
    for specification_index, specification in enumerate(core_specifications()):
        for c_index, regularization_c in enumerate(
...
def test_worker_publishes_models_and_resumes_from_immutable_shard(
    ...
    frames = {
        year: training_frame.assign(year=year) for year in range(2004, 2009)
    }
...
def test_finalize_refits_declared_winner_on_2004_2007(
    ...
    frames = {
        year: training_frame.assign(year=year) for year in config.TRAIN_YEARS
    }
    monkeypatch.setattr(
        "hmda_seconds.model_selection_cluster.model_selection.load_selection_years",
    ...
    assert finalize_selection(decision, data_dir="unused", model_output=output) == output
