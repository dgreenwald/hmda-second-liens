#!/usr/bin/env python3
"""Generate one stage of the HMDA-only boosting selection Slurm array."""
...
def main() -> None:
    ...
    destination = args.destination or (
        config.OUTPUT_DIR / "slurm" / "hmda_only_boosting" / stage_label
    )
    ...
    manifest, script = write_slurm_array(
        ...
        memory=args.memory,
        job_name=f"hmda-only-boost-{stage_label}",
