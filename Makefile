...
EXPORT_PORTABLE_LOGISTIC_FLAGS ?=
SYNC_CLUSTER_FLAGS ?=
...
export-portable-logistic:
	$(PYTHON) $(SCRIPTS_DIR)/export_portable_logistic.py $(EXPORT_PORTABLE_LOGISTIC_FLAGS)
...
historical-model-family-cluster historical-model-family-cluster-submit:
	$(PYTHON) $(SCRIPTS_DIR)/run_historical_model_family_cluster.py \
		$(HISTORICAL_MODEL_FAMILY_FLAGS) \
		$(if $(filter historical-model-family-cluster-submit,$@),--submit,)

aggregate-historical-model-family:
	$(PYTHON) $(SCRIPTS_DIR)/aggregate_historical_model_family.py --manifest $(OUTPUT_DIR)/slurm/historical_model_family/historical_model_family_jobs.json
