...
sync-selection-results:
	$(PYTHON) $(SCRIPTS_DIR)/sync_cluster_results.py $(SYNC_CLUSTER_FLAGS)
...
generate-model-family-comparison:
	$(PYTHON) $(SCRIPTS_DIR)/generate_model_family_comparison_slurm.py $(MODEL_FAMILY_COMPARISON_FLAGS)

submit-model-family-comparison:
	$(PYTHON) $(SCRIPTS_DIR)/generate_model_family_comparison_slurm.py --submit

aggregate-model-family-comparison:
	$(PYTHON) $(SCRIPTS_DIR)/aggregate_model_family_comparison.py --manifest $(OUTPUT_DIR)/slurm/model_family_comparison/density_ratio_jobs.json

aggregate-model-family-comparison-run:
	$(if $(strip $(MODEL_FAMILY_COMPARISON_RUN_ID)),,$(error MODEL_FAMILY_COMPARISON_RUN_ID is required))
	$(PYTHON) $(SCRIPTS_DIR)/aggregate_model_family_comparison.py --manifest $(OUTPUT_DIR)/slurm/model_family_comparison_runs/$(MODEL_FAMILY_COMPARISON_RUN_ID)/density_ratio_jobs.json

model-family-comparison-cluster model-family-comparison-cluster-submit:
	$(PYTHON) $(SCRIPTS_DIR)/run_model_family_comparison_cluster.py \
		$(MODEL_FAMILY_CLUSTER_FLAGS) \
		$(if $(filter model-family-comparison-cluster-submit,$@),--submit,)
